...
class DiffusionWorldInterpolatorWModel(DiffusionV2WModel):
    ...
    def denoise(self, xt: torch.Tensor, sigma: torch.Tensor, condition: VideoExtendCondition) -> DenoisePrediction:
        ...
        condition_dict = {
            k: v.to(self.precision) if isinstance(v, torch.Tensor) else v
    ...
    def _augment_image_dim_inplace(self, data_batch: Dict[str, Tensor]) -> None:
        if self.input_image_key in data_batch:
            if IS_PREPROCESSED_KEY not in data_batch or not data_batch[IS_PREPROCESSED_KEY]:
                data_batch[self.input_image_key] = rearrange(data_batch[self.input_image_key], "b c h w -> b c 1 h w").contiguous()
                data_batch[IS_PREPROCESSED_KEY] = True
    ...
    def add_condition_video_indicator_and_video_input_mask(
        ...
        log.debug(f"condition_location first_n, num_condition_t {num_condition_t}, condition.video_cond_bool {condition.video_cond_bool}")
    ...
    def _get_conditions(
        ...
        condition = self.add_condition_video_indicator_and_video_input_mask(condition_latent, condition, num_condition_t)
        uncondition.video_cond_bool = False if add_input_frames_guidance else True
        uncondition = self.add_condition_video_indicator_and_video_input_mask(condition_latent, uncondition, num_condition_t)
